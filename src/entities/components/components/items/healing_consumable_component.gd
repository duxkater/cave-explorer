...
func activate(action: ItemAction) -> bool:
	...
	if amount_recovered > 0:
		consumer.game.get_log_manager().add_message("You consume the %s, and recover %d HP!" % [entity.get_entity_name(), amount_recovered], LoggerColors.HEALTH_RECOVERED)
		consume(consumer)
		return true
	consumer.game.get_log_manager().add_message("Your health is already full.", LoggerColors.HEALTH_RECOVERED)
	return false
